...
class FBManager: NSObject {
    ...
    /// check if we have the all granted permission in our app like public_profile, email
    ///
    /// - Returns: true/false
    func hasGrantedPermission()-> Bool{
        if     FBSDKAccessToken.current().hasGranted("public_profile") &&  FBSDKAccessToken.current().hasGranted("email"){
            return true
        }
        return false
    }
    ...
    func  loginTofacebook(presentOnVC:UIViewController!,completion:@escaping ((Bool,Error?)->Void))  {
        if loginVC == nil {
            ...
            self.loginVC?.logIn(withReadPermissions: ["public_profile", "email"], from: presentOnVC) { (result, error) in
                if error != nil{
    ...
    func fetchUseData(fbUserDetailCompletion:@escaping (User?,Error?)->Void){
                ...
                if ((error) == nil) {
                    if let fbResult = result as? [String:Any]{
                        if let _ = fbResult["email"]{
                            let user : User? = Mapper<User>().map(JSON: fbResult )
                            fbUserDetailCompletion(user, nil)
                        }else {
                            fbUserDetailCompletion(nil, NSError.init(domain: "com.evon.sampleCode", code: 400, userInfo: ["msg":"Email Not found"]))
                        }
                    }
                }
                else {
